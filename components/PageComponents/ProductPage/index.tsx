...
function ProductPage() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:5082/api/SubCategory/get/id?id=${selectedSubCategoryId}`
      )
      .then((response) => {
      ...
      if (searchQuery.trim() !== "" || selectedSubCategoryId) {
        ...
        try {
          const response = await axios.post(
            "http://localhost:5082/api/Product/PostAllWithPaginaiton",
            {
              name: searchQuery,
    ...
    <div className="flex flex-col md:flex-row max-w-full mx-auto bg-white shadow-md overflow-hidden">
      ...
      >
        {loading ? (
          ...
          <>
            <div className=" flex flex-col h-48 rounded-lg justify-center items-center py-36">
              <SearchBar
                onSearch={() => {}}
            ...
            <div>
              <p className="italic  text-[48px]">{subcategories?.name}</p>
              <span className="pt-2">{subcategories?.description}</span>
            </div>
            <div>{selectedProductId && <Popup />}</div>
            <div
        ...
        {products && (
          <Pagination
            ...
            classNames={{
              ...
              cursor:
                "w-10 h-10 flex items-center justify-center rounded-full bg-gray-200 text-gray-700 hover:bg-red-300",
              chevronNext: "",
